...
Texture* Texture::load(const char* filename, bool mipmaps)
{
	...
	if (ext == ".tga" || ext == ".TGA")
	{
		...
		TGAInfo* tgainfo = tex->loadTGA(filename);
		if (tgainfo == NULL)
			return false;

		//How to store a texture in VRAM
		glGenTextures(1, &tex->texture_id); //we need to create an unique ID for the texture
		glBindTexture(GL_TEXTURE_2D, tex->texture_id);	//we activate this id to tell opengl we are going to use this texture
		...
		else
		{
			...
			{
				glTexImage2D(GL_TEXTURE_2D, 0, ( tgainfo->bpp == 24 ? 3 : 4), tgainfo->width, tgainfo->height, 0, ( tgainfo->bpp == 24 ? GL_BGR : GL_BGRA), GL_UNSIGNED_BYTE, tgainfo->data); //upload without mipmaps
				tex->generateMipmaps(); //glGenerateMipmapEXT(GL_TEXTURE_2D);
			}
			else //use old way
			{
		...
		tex->width = tgainfo->width;
		tex->height = tgainfo->height;
